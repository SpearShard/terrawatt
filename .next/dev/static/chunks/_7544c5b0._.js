...
function Video() {
    ...
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useEffect"])({
        "Video.useEffect": ()=>{
            ...
            const jumpToTerramart = {
                "Video.useEffect.jumpToTerramart": ()=>{
                    const targetFrame = 589;
                    const progress = targetFrame / (FG_FRAMES - 1);
                    const st = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$gsap$2f$ScrollTrigger$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["ScrollTrigger"].getAll().find({
                        "Video.useEffect.jumpToTerramart.st": (t)=>t.trigger === containerRef.current
                    }["Video.useEffect.jumpToTerramart.st"]);
                    if (!st) return;
    ...
    //   if (action === "go_mart") {
    //     localStorage.removeItem("TW_action");
    //     // ✅ Phase 1: scroll to video section
    //     const section = containerRef.current;
    //     if (!section) return;
    //     section.scrollIntoView({ behavior: "smooth" });
    //     // ✅ Phase 2: AFTER we reach the section, jump frames
    //     setTimeout(() => {
    //       requestAnimationFrame(jumpToTerramart);
    //     }, 700); // small delay to ensure ScrollTrigger settles
    //   }
    //   // Same-page trigger (Pulse / TerraCharge → TerraMart)
    //   window.addEventListener("triggerVideoJump", jumpToTerramart);
    //   return () => {
    //     window.removeEventListener("triggerVideoJump", jumpToTerramart);
    //   };
...
_s(Video, "8N8ET+8gws0WwO5pzUGk2ilmFwQ=");
_c = Video;
var _c;
