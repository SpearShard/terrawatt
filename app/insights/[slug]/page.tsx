...
export default function BlogPost({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const sortedImages = blog.images
    ?.slice()
...
function ImageRevealStack({ images }: { images: BlogImage[] }) {
  ...
  return (
    <div className="relative w-full mb-32 h-[60vh] max-w-5xl mx-auto">

      {images.map((img: BlogImage, index: number) => {
